...
const BlackScr = (props) => {
  ...
  const ex = useSelector((state) =>
    state.chart[0]?.dataset.map((el) => [el.time, el.BlackScr])
  );

  const BlackScr2 = useSelector((state) =>
    state.chart[0]?.dataset.reduce((sum, el) => sum + el.BlackScr)
  );
  ...
  const options = {
    ...
    series: [{ name: "data", data: BlackScr }],
  };
  return (
    <form>
